...
const Navbar = () => {
  ...
  //get the auth status from local storage
  useEffect(() => {
  ...
  //it will maintain login when authtoken is present for all routes
  useEffect(() => {
    setIsLoggedIn(!!localStorage.getItem('authToken'));
    setIsOpen(false);
  }, [location]);
    ...
    <div className = "px-4 pt-3 pb-4 space-y-2">
      <div className = {styles.mobileGrid}>
        {navLinks.map((link) => (
          ...
          className = {`${styles.mobileMenu.base} ${
            isActive(link.to)
             ? styles.mobileMenu.active 
             : styles.mobileMenu.inactive
          }`}
          >
            {link.label}
          </Link>
