...
const bookingSchema = new Schema(
  {
    userId: { type: Schema.Types.ObjectId, ref: "User", required: true }, //coming from User model
    ...
    stripeSession: { type: Schema.Types.Mixed, default: {} }, //Mixed type field: stripeSession (useful for storing any structure from Stripe)
  },
  { timestamps: true }
); //customer details and shipping address with payment details
...
bookingSchema.pre('validate', async function(next){
    ...
    try{
        const carDoc = await Car.findById(this.car.id).lean(); //lean() -> Returns a plain JS object instead of a Mongoose document → faster.
         if (carDoc) {
    ...
    try{
        ...
        if(blockingStatuses.includes(doc.status)){
            ...
            await Car.findByIdAndUpdate(
                carId,
                {
                    $pull: {bookings: {bookingId: doc_id}}
                },
                {new : true}
            ).exec();
        }
