...
//Deletes old image files from /uploads if a booking updates its image.
...
export const createBooking = async (req, res) => {
    ...
    try{
        ...
        if(Number.isNaN(pickup.getTime()) || Number.isNaN(ret.getTime()) || ret < pickup){
            await session.abortTransaction();
            session.endSession();
            return res.status(400).json({ success: false, message: 'Invalid pickupDate or returnDate' });
        }
    ...
    return res.status(201).json({ success: true, message: 'Booking created successfully', booking: saved });
...
//get function: Fetches all bookings with: Pagination, Search, Status filter, Car filter, Date-range filter
...
//get booking for a particular user(logged-in user only.)
